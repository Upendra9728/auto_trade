...
export default function SignalDetailScreen() {
  ...
  const hasActivePlaced = (notifPage?.items ?? []).some(
    n => n.status === 'placed' && !TERMINAL_LIVE.has(n.live_status ?? '')
  );
  ...
  const totalPages = notifPage?.meta.total_pages ?? 1;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Modal visible={modifyVisible} transparent animationType="slide" onRequestClose={() => setModifyVisible(false)}>
        ...
        <View style={styles.modifySheet}>
          ...
          <ModifyField label="Entry Price (PENDING only)" value={modifyForm.price} onChangeText={(v) => setModifyForm(f => ({ ...f, price: v }))} />
          <ModifyField label="Target Price" value={modifyForm.target_price} onChangeText={(v) => setModifyForm(f => ({ ...f, target_price: v }))} />
          <ModifyField label="Stop-Loss Price" value={modifyForm.stop_loss_price} onChangeText={(v) => setModifyForm(f => ({ ...f, stop_loss_price: v }))} />
          <ModifyField label="Trailing Jump" value={modifyForm.trailing_jump} onChangeText={(v) => setModifyForm(f => ({ ...f, trailing_jump: v }))} />
